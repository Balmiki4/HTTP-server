...
//
// COMMENTS:
//FindNode: Traverses the Tree based on the charactes in the prefix and 
//          returns the TreeNode that we end at.
...
//     for child in node.children:
//         CollectWords(child, prefix + child.value, results)

// COMMENTS:
//CollectWords: Collects all the words starting from a given TreeNode.
...
// function GetCandidates(root, prefix):
//     candidates = []
//     prefixNode = FindNode(root, prefix, 0)
//     CollectWords(prefixNode, prefix, candidates)
//     return candidates

// COMMENTS:
//GetCandidates:   Returns the list of all possible words that can be made starting with
//         the letters in prefix, based on traversing the tree with the given root.
